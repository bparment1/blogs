...
#zonal_var_df <- as.data.frame(t(zonal(x=r_var,z=r_ref,fun='mean')))
zonal_var_df <- as.data.frame(zonal(x=r_var,z=r_ref,fun='mean'))
head(zonal_var_df)
View(zonal_var_df)

test <- gather_(zonal_var_df,key="variable",value="value",names(r_var))
test$date <- unlist(lapply(test$variable,FUN=function(x){strsplit(x,'["_"]')[[1]][2]}))
#test$index <- unlist(lapply(test$variable,FUN=function(x){strsplit(x,'["_"]')[[1]][1]}))
test$zone <- as.factor(test$zone)
#test$index <- as.factor(test$index)
#
View(test)
xyplot(value~date|zone,groups="index",data=test, type = c('g', 'p'))
xyplot(value~date|zone,data=test)

xyplot(value~date|index,groups="zone",data=test, type = c('g', 'p'))
test$index
View(test)
xyplot(value~date|zone,data=test)

plot(value~index,data=subset(test,zone=1))
plot(value~index,data=subset(test,zone=0))

plot(value~date,data=subset(test,zone=1))
?xyplot

View(test)
names(zonal_var_df)
?gather
plot(zonal_var_df$V1)
?mutate
View(NDVI_df)
...
plot()
xtab_tb <- crosstab(r_after_flood,r_ref)

r_overlap <- r_ref_rec * r_after_flood
plot(r_overlap) ## Flood map area of aggreement with FEMA


plot(r_ref) # zone 1 is flooded and zone 0 is not flooded
...
### 1) Simple thresholding: Check for low NDVI

r_rec_before <- r_before_NDVI < 0.2
r_rec_after <- r_before_NDVI < 0.2

### THis is suggesting flooding!!!
plot(r_rec_before, main="Before RITA, NDVI < 0.2")
plot(r_rec_after, main="After RITA, NDVI < 0.2")
...
r_std <- (r_diff - )/cellStats(r_diff,"sd")
...
r_change_pos <- r_std > threshold_val[1]
...
r_impact <- reclassify(r_diff,rclmat)
