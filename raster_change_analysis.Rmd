...
#var_df <- as.data.frame(stack(r_var,r_diff,r_ref))
#names(var_df) <- c("NDVI_1","NDVI_2","diff","zone")
#boxplot(diff ~ zone,data=var_df)
#mod_val <- lm(diff ~ zone,data=var_df)
#plot(mod_val)
...
plot(r_impact, legend = FALSE, col = c("grey","yellow","red"))
legend("topright", legend = c("No change", "level 1", "level 2"), 
       fill = c("grey","yellow","red"))
