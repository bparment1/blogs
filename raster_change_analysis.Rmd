...
We generate two different impact maps with two reclassificiation procedures:
- based on the original difference values
- based on the standardized difference values

In the first procedure, we assign positive and low decreases in NDVI value 0 (no change), level 1 decrease in NDVI (low intensity) for range [-0.3,0.1] while level 2 decrease (higher impact from higher intensity).

We keep the same labels in the second procedure but use different thresholds. We standarize the difference image using the mean and standard deviation values. We generate the three classes using different threshold assuming a normal distribution as often shown in the scientific literature. Both methods are summarized in the table below:

no change: [-1.64, 10] 0
low impact: [-1.96, -1.64]
high impact: [-10, -1.96]
